...
func ParseModel(path string, modelName string) (*StorageResource, error) {
	content, err := os.ReadFile(path)
	if err != nil {
		return nil, err
	}

	// Update the regex to match the specific modelName
	regex, err := regexp.Compile(fmt.Sprintf(`type %s\s+struct\s*{([^}]+)}`, modelName))
	if err != nil {
		return nil, err
	}
	...
	// The struct fields are captured in matches[1]
	expression := matches[1]

	// Parse the fields from the expression
	fields := []Field{}
	fieldLines := strings.Split(expression, "\n")
	for _, line := range fieldLines {
		...
		// Split the line into field name and type
		...
		// Optional: Break if you encounter specific field names
